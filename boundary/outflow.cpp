...
void Outflow::ApplyBoundary(Grid& grid)
{
	...
	switch (this->side)
	{
		...
		case Side::x2b:
		{
			for (std::size_t i = 0; i < grid.GetNGhost(Axis_x2); i++)
			{
				consvars(Range::all(), i, Range::all(), Range::all()) =
				 consvars(Range::all(), grid.GetNGhost(Axis_x2), Range::all(), Range::all());
			}
			break;
		}
		case Side::x2e:
		{
			for (std::size_t i = consvars.extent(1)-grid.GetNGhost(Axis_x2); i < consvars.extent(1); i++)
			{
				consvars(Range::all(), i, Range::all(), Range::all()) =
				 consvars(Range::all(), consvars.extent(1)-grid.GetNGhost(Axis_x2)-1, Range::all(), Range::all());
			}
			break;
		}
		case Side::x3b:
		...
		case Side::x3e:
		{
			for (std::size_t i = consvars.extent(2)-grid.GetNGhost(Axis_x3); i < consvars.extent(2); i++)
			{
				consvars(Range::all(), Range::all(), i, Range::all()) =
